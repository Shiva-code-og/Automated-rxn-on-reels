...
AutoReact is a modern, premium web-based dashboard and automated worker that monitors incoming Instagram Direct Messages (DMs) for shared Reels, fetches their comments, analyzes the content sentiment using Google Gemini AI, and automatically reacts with the contextually appropriate emoji.
...
1. **Connect Instagram:**
   ...
   * *Note: AutoReact saves your login session locally to `session.json` to prevent repeated logins.*
...
2. **Configure Gemini AI:**
   ...
   * *Note: This will securely write `GEMINI_API_KEY=your_key` to a local `.env` file in the project root.*
...
- **`data.db`** (local SQLite database containing reaction logs)
- **`session.json`** (encrypted Instagram login session cookies)

*Never share your `.env` or `session.json` files with anyone.*
